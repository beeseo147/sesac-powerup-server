...
size_t Room::size() const { return m_HostMap.size(); }
...
map<int32_t, bool> Room::get_players_ready() const { return m_PlayersReady; }
...
map<int32_t, int32_t> Room::get_players_rank() const {
  ...
  for (auto it = m_PlayersPoint.begin(); it != m_PlayersPoint.end(); it++) {
    ...
    int32_t rank = 1;
    for (auto it2 = m_PlayersPoint.begin(); it2 != m_PlayersPoint.end();
         it2++) {
...
bool Room::is_game_started() const { return m_GameStarted; }
...
void Room::start_game() { m_GameStarted = true; }

void Room::end_game() { m_GameStarted = false; }