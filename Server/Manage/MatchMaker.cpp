...
{
    for (auto it = instance().m_GroupRoomMap.begin(); it != instance().m_GroupRoomMap.end(); it++)
    {
        if (it->second.size() < MAX_PLAYERS_PER_ROOM && !it->second.is_game_started())
            return it->first;
    }
